...
export const updateUserProfile = async(request, response) => {
    try {
        const {fullName, username, currentPassword, newPassword , email, bio, link} = request.body;
        ...
        if(!currentPassword && newPassword || !newPassword && currentPassword){
            return response.status(400).json({error: "Please provide both current and new password"})
        }
